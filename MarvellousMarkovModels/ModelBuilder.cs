...
public class ModelBuilder
{
    ...
    public readonly Dictionary<string, Dictionary<string, int>> _productions = new();

    public readonly Dictionary<string, int> _startingStrings = new();
    ...
    public ModelBuilder Teach(string example)
    {
        ...
        for (var i = 0; i < example.Length - _order + 1; i++)
        {
    ...
    {
        if (!_productions.TryGetValue(key, out var set))
        {
